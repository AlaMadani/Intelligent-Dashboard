...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--project-root", required=True, help="root folder of the Java project scanned for @ActionTracking")
    parser.add_argument("--output", default="audit_trail_2025.csv", help="output file (.csv or .txt)")
    parser.add_argument("--start-date", default="2025-01-01", help="dataset start date (inclusive)")
    parser.add_argument("--end-date", default="2025-12-31", help="dataset end date (inclusive)")
    parser.add_argument("--users", type=int, default=50, help="number of simulated users")
    parser.add_argument("--actions-per-month", type=int, default=5000, help="total number of actions per month")
    parser.add_argument("--seed", type=int, default=2025)
    parser.add_argument("--created-at-format", choices=["java", "kibana"], default="kibana")
    parser.add_argument("--file-format", choices=["csv", "txt"], default=None, help="override format inferred from output extension")
...
def random_time_in_month(month_start: datetime, month_end: datetime, rng: random.Random) -> datetime:
    # Bias toward business days and typical traffic peaks while staying realistic.
    days = []
    cursor = month_start
...
def choose_session_lengths(total_actions: int, rng: random.Random) -> List[int]:
    ...
    while remaining > 0:
        ...
        # Keep enough actions for future sessions.
...
def choose_status(category: str, rng: random.Random) -> str:
    failure_probability = {
        "login": 0.03,
        "logout": 0.01,
        "banking": 0.08,
...
def event_delay_seconds(category: str, rng: random.Random) -> int:
    ranges = {
        "login": (15, 90),
        "dashboard": (20, 180),
        "insured": (60, 420),
...
def build_session_categories(length: int, rng: random.Random) -> List[str]:
    ...
    template = list(rng.choice(patterns))
    categories = ["login"]
...
def pick_action_for_category(catalog: ActionCatalog, category: str, used_keys: Set[str], rng: random.Random) -> ActionDef:
    ...
    if action is None:
        action = catalog._pick_underused(["insured", "open", "document", "refund", "banking", "contact", "dashboard", "other", "login", "logout"], exclude_keys=used_keys)
    if action is None:
...
def build_month_plans(
    ...
    for length in session_lengths:
        ...
        for seq_idx, category in enumerate(categories, start=1):
            ...
            plans.append(SessionEventPlan(action=action, timestamp=current_time, sequence_in_session=seq_idx, session_number=session_number))
    ...
    plans.sort(key=lambda plan: plan.timestamp)
    return plans
...
def build_event_record(
    ...
    if sequence_in_session == 1 or category == "login" or user.session_id is None:
        user.session_id = str(rng.randint(100000, 9999999))
    maybe_rotate_ip(user, rng)
    status = choose_status(category, rng)
    ...
    event = {
        ...
        "createdAt": format_created_at(timestamp.astimezone(timezone.utc), created_at_format),
        ...
        "sequenceInSession": sequence_in_session,
    }
...
def main(argv: List[str]) -> int:
    ...
    if len(months) != 12:
        print("warning: the selected date range does not cover exactly 12 calendar months", file=sys.stderr)

    if args.actions_per_month % args.users != 0:
        print("actions-per-month must be divisible by users to keep counts equal", file=sys.stderr)
        return 2
        ...
        for user in users:
            ...
            for plan in plans:
                event = build_event_record(
                    ...
                    sequence_in_session=plan.sequence_in_session,
    ...
    summary = {
        ...
        "users": args.users,
        "actions_per_user_per_month": actions_per_user_per_month,
        "actions_per_user_per_year": annual_target_per_user,
        "unique_actions_scanned": len(actions),
    }
