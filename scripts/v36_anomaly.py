...
TAGS=['credential_stuffing','session_hijacking','data_exfiltration','api_scraping','off_hours_compromise','behavioral_sequence_anomaly']
def anomaly_session(user,start,tag,args,rng,lookup):
    user.sessions+=1; sid=f'anom-{user.insured_id}-{user.sessions:04d}-{uuid.uuid4().hex[:6]}'; events=[]; ts=start; prev=None; seq=1
    events.append(make_event(user,sid,seq,ts,prev,start,'POST','/auth/login','login','submit_credentials',lookup,rng,200,0,'')); prev=ts; seq+=1
    if tag=='credential_stuffing': specs=[('POST','/auth/login','login','submit_credentials',rng.choice([401,401,403,429])) for _ in range(rng.randint(6,16))]
    elif tag=='session_hijacking':
        user.country,user.region=rng.choice(FOREIGN); user.ip=make_ip(rng,user.country); user.device=rng.choice(['desktop','mobile']); user.browser=rng.choice(['chrome','firefox','edge']); user.os='windows' if user.device=='desktop' else 'android'
        specs=[('GET','/all-info?actionKey=requests','home','refresh_requests',200),('GET','/documents','documents','list_documents',200),('GET','/documents/file','documents','download_document',200)]
    elif tag=='data_exfiltration': specs=[('GET',rng.choice(['/documents/file','/document-display/download/','/insured/all-refund-resume']),'documents','bulk_download',200) for _ in range(rng.randint(3,8))]
    elif tag=='api_scraping': specs=[('GET',rng.choice(['/documents','/documents/file','/requests/messages/file','/all-info?actionKey=documents']),'documents','scrape',200) for _ in range(rng.randint(5,10))]
    elif tag=='off_hours_compromise':
        ts=ts.replace(hour=rng.randint(1,4),minute=rng.randint(0,59)); prev=ts-timedelta(seconds=1); start=prev
        specs=[('GET','/documents','documents','list_documents',200),('GET','/documents/file','documents','download_document',200),('POST','/insured/refund-resume','refunds','download_refund_pdf',200)]
    else: specs=[('GET','/documents/file','documents','rapid_download',200),('PUT','/insured/rib','bankinginformation','update_rib',200),('GET','/requests/messages/file','requests','rapid_request_file',200),('POST','/auth/login','login','unexpected_login',401)]
    for method,path,page,front,code in specs:
        delay=rng.randint(8,80) if tag in ('credential_stuffing','behavioral_sequence_anomaly') else rng.randint(20,300); ts=prev+timedelta(milliseconds=delay); event=make_event(user,sid,seq,ts,prev,start,method,path,page,front,lookup,rng,code,1,tag)
        if tag=='data_exfiltration': event['response_data_size_bytes']=rng.randint(2500000,9000000)
        elif tag=='api_scraping': event['response_data_size_bytes']=rng.randint(50000,900000)
        events.append(event); prev=ts; seq+=1
    if should_end(args,rng):
        method,path,page,front=end_spec(args,rng); ts=prev+timedelta(seconds=rng.randint(args.session_end_delay_min,args.session_end_delay_max)); events.append(make_event(user,sid,seq,ts,prev,start,method,path,page,front,lookup,rng,200,0,''))
    return events

def main():
    p=argparse.ArgumentParser(description='Corrected V3.6 anomaly live simulator'); add_common(p); p.set_defaults(insured_count=100,session_end_probability=.40,session_end_policy='auto')
    p.add_argument('--tag',choices=['all']+TAGS,default='all'); p.add_argument('--session-count',type=int,default=0,help='0 means continuous until Ctrl+C'); p.add_argument('--concurrent-sessions',type=int,default=4); p.add_argument('--rate',type=float,default=90,help='events per minute pacing target'); p.add_argument('--virtual-start',default='now')
    args=p.parse_args(); rng=random.Random(args.seed); lookup=build_lookup(load_json(args.backend_apis)); load_json(args.actions_order); users=make_users(rng,args.insured_count); sink=KafkaSink(args); tags=TAGS if args.tag=='all' else [args.tag]; count=0; virtual=parse_dt(args.virtual_start)
    try:
        while args.session_count==0 or count<args.session_count:
            user=users[count%len(users)]; tag=tags[count%len(tags)]; events=anomaly_session(user,virtual,tag,args,rng,lookup)
            for event in events:
                sink.send(event); time.sleep(max(.001,60/max(args.rate,.001)/max(len(events),1)))
            count+=1; virtual=max(parse_dt(events[-1]['timestamp']),virtual)+timedelta(seconds=1)
    except KeyboardInterrupt: print('\nShutdown requested.',file=sys.stderr,flush=True)
    finally:
        sink.close()
        if args.print_summary: print(json.dumps({'mode':'anomaly','sessions':count,'attempted':sink.attempted,'delivered':sink.delivered,'failed':sink.failed},indent=2),file=sys.stderr)
