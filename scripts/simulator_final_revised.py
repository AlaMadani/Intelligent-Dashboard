...
Improvements over the original:
  • Loads the tracked action catalog from backend-apis-actions.json.
  • Loads normal navigation flows from actions_order-v2.json.
  • Each user follows a realistic route-based session: auth -> [routes] -> logout.
  • Events include 'route' and 'prevAction' fields for SpringBoot demos.
  • Supports --anomaly-mode to inject illogical sequences for model testing.
"""
import argparse
import json
import math
...
# ---------------------------------------------------------------------------
# Fallback embedded catalog kept for backwards compatibility.
# Runtime startup replaces it from the JSON-driven training config.
# ---------------------------------------------------------------------------
...
# Runtime state, rebuilt from the JSON-driven training configuration.
...
LOGIN_TYPE_WEIGHTS: List[Tuple[str, float]] = [
    ("direct", 0.58),
    ("mfa", 0.25),
    ("sso", 0.10),
    ("mfa_regen", 0.04),
...
def rebuild_runtime_catalog(
    backend_apis_path: str,
    actions_order_path: str,
) -> Dict[str, int]:
    """Align this simulator with the JSON-driven training configuration."""
    global ALL_ACTIONS, ACTION_BY_VALUE, ACTIONS_BY_ROUTE, LOGIN_TYPE_WEIGHTS, ROUTE_WEIGHTS

    catalog = training_sim.load_actions_from_json(backend_apis_path)
    nav_summary = training_sim.initialize_navigation(actions_order_path, backend_apis_path, catalog)

    auth_values: List[str] = []
    for sequence in training_sim.LOGIN_SEQUENCES.values():
        auth_values.extend(sequence)
    logout_values: List[str] = []
    for sequence in training_sim.LOGOUT_BY_LOGIN.values():
        logout_values.extend(sequence)

    all_actions: List[RouteAction] = []
    action_by_value: Dict[str, RouteAction] = {}
    actions_by_route: Dict[str, List[RouteAction]] = defaultdict(list)

    for value in dedupe_preserve_order(auth_values):
        action_def = catalog.get(value)
        if action_def is None:
            continue
        route_action = RouteAction(value, action_def.type_name, action_def.sub_type, "auth")
        all_actions.append(route_action)
        action_by_value[value] = route_action
        actions_by_route["auth"].append(route_action)

    for value in dedupe_preserve_order(logout_values):
        action_def = catalog.get(value)
        if action_def is None:
            continue
        route_action = RouteAction(value, action_def.type_name, action_def.sub_type, "logout")
        all_actions.append(route_action)
        action_by_value[value] = route_action
        actions_by_route["logout"].append(route_action)

    for route_name, weighted_actions in training_sim.ROUTE_ACTIONS.items():
        for value, _ in weighted_actions:
            action_def = catalog.get(value)
            if action_def is None:
                continue
            route_action = RouteAction(value, action_def.type_name, action_def.sub_type, route_name)
            all_actions.append(route_action)
            actions_by_route[route_name].append(route_action)
            action_by_value.setdefault(value, route_action)

    login_totals: Dict[str, float] = defaultdict(float)
    route_totals: Dict[str, float] = defaultdict(float)
    for persona_cfg in training_sim.PERSONAS.values():
        persona_weight = persona_cfg["weight"]
        for login_type, weight in persona_cfg["login_probs"].items():
            if training_sim.LOGIN_SEQUENCES.get(login_type):
                login_totals[login_type] += persona_weight * weight
        for route_name, weight in persona_cfg["route_probs"].items():
            if actions_by_route.get(route_name):
                route_totals[route_name] += persona_weight * weight

    ALL_ACTIONS = all_actions
    ACTION_BY_VALUE = action_by_value
    ACTIONS_BY_ROUTE = actions_by_route
    if login_totals:
        LOGIN_TYPE_WEIGHTS = list(login_totals.items())
    if route_totals:
        ROUTE_WEIGHTS = list(route_totals.items())

    return nav_summary


# ---------------------------------------------------------------------------
# Anomaly injection patterns for model testing
# ---------------------------------------------------------------------------
ANOMALY_PATTERNS = {
    "wrong_sequence": [
        # Logout before login
        ...
        ["Exporter remboursement", "Connexion"],
        # Double login
        ["Connexion", "Connexion", "Déconnexion"],
    ],
    "wrong_route": [
        # Banking action on home page
        ("Changer les coordonnées bancaire", "home"),
        # Auth action on a content page
        ("Connexion", "documents"),
        # Logout action mid-session with non-logout action after
        ("Déconnexion", "requests"),
    ],
    "rapid_fire": None,   # many events in < 5 seconds
    "impossible_ip_hop": None,  # different continents within 1 second
}
...
def choose_http_code(status, route, action_value, rng):
    if status == "OK":
        ...
        if action_value in ("Connexion SSO","Connexion en tant que"):
            return "200"
        if route in ("documents","contact_general","claims","beneficiaries","banking","resiliation"):
            return str(rng.choice([200,201,202]))
        return str(rng.choice([200,204]))
    failure_codes = {
        "auth": [401, 403],
        "banking": [400, 409, 422],
        "contact_general": [400, 422],
        "claims": [400, 422],
        "documents": [400, 422],
        "resiliation": [400, 422],
    }
...
def build_request_data(action: RouteAction, user: UserProfile, rng: random.Random) -> Optional[str]:
    ...
    if r == "refunds" and "remboursement" in action.value.lower():
...
def plan_new_session(user: UserProfile, rng: random.Random, anomaly_mode: bool) -> None:
    """Fill user.action_queue with the next session's actions."""
    ...
    # Normal session: login → routes → logout
    login_type = weighted_choice(LOGIN_TYPE_WEIGHTS, rng)
    for value in training_sim.LOGIN_SEQUENCES.get(login_type, ["Connexion"]):
        action = ACTION_BY_VALUE.get(value)
        if action is not None:
            user.action_queue.append((action, "auth"))
    ...
    for _ in range(rng.randint(1, 4)):
        ...
        depth = min(depth, len(pool))
        for act in rng.sample(pool, depth):
            user.action_queue.append((act, route))

    # Logout
    for value in training_sim.LOGOUT_BY_LOGIN.get(login_type, ["Déconnexion"]):
        action = ACTION_BY_VALUE.get(value)
        if action is not None:
            user.action_queue.append((action, "logout"))


def _inject_anomaly_session(user: UserProfile, rng: random.Random) -> None:
    """Inject an anomalous session for testing the anomaly detection model."""
    login_action = ACTION_BY_VALUE.get("Connexion")
    logout_action = ACTION_BY_VALUE.get("Déconnexion") or ACTION_BY_VALUE.get("SSO Disconnect")
    pattern = rng.choice([
        "out_of_order",    # action before login
        "double_login",    # login twice without logout
        "wrong_route",     # action on implausible route
        "post_logout",     # action after explicit logout
    ])

    if pattern == "out_of_order":
        # Start with a content action without being logged in
        pool = ACTIONS_BY_ROUTE.get("documents", []) + ACTIONS_BY_ROUTE.get("contact_general", [])
        if pool:
            act = rng.choice(pool)
            user.action_queue.append((act, act.route))
        if login_action is not None:
            user.action_queue.append((login_action, "auth"))

    elif pattern == "double_login" and login_action is not None:
        user.action_queue.append((login_action, "auth"))
        user.action_queue.append((login_action, "auth"))
        if ACTIONS_BY_ROUTE.get("refunds"):
            user.action_queue.append((rng.choice(ACTIONS_BY_ROUTE["refunds"]), "refunds"))
        if logout_action is not None:
            user.action_queue.append((logout_action, "logout"))

    elif pattern == "wrong_route" and login_action is not None:
        # Banking action reported on a refund-focused route
        user.action_queue.append((login_action, "auth"))
        if ACTIONS_BY_ROUTE.get("banking"):
            act = rng.choice(ACTIONS_BY_ROUTE["banking"])
            wrong = RouteAction(act.value, act.type_name, act.sub_type, "refunds")
            user.action_queue.append((wrong, "refunds"))
        if logout_action is not None:
            user.action_queue.append((logout_action, "logout"))

    elif pattern == "post_logout" and login_action is not None:
        user.action_queue.append((login_action, "auth"))
        if logout_action is not None:
            user.action_queue.append((logout_action, "logout"))
        # Action after logout - anomalous
...
def generate_event(action: RouteAction, route: str, user: UserProfile,
    ...
    event = {
        ...
        "requestReturn":   json.dumps({"status":"success","ref":f"REF-{rng.randint(100000,999999)}"}) if status=="OK" else json.dumps({"status":"error"}),
        ...
        "route":           route,           # ← new field for SpringBoot consumer
        "prevAction":      user.prev_action, # ← new field
    }
    ...
    if action.route == "auth" and action.value in ("Connexion","Connexion SSO","Connexion en tant que","Activation de compte"):
        user.connected = True
        user.actions_in_session = 0
...
def build_users(rng: random.Random, count: int) -> List[UserProfile]:
    pools = {"co":[27011,27012,27013,27021],"cg":[12001,12002,12003],
             "cs":[110027,110028,110029],"ins":[1,2,3],"ic":[302,303,304],
             "hn":[1,2,3],"dom":[10,11,12],"env":[10,11]}
    ...
    for _ in range(count):
        ...
        u = UserProfile(
            ...
            company_id=rng.choice(pools["co"]), company_group_id=rng.choice(pools["cg"]),
            company_section_id=rng.choice(pools["cs"]), insurer_id=rng.choice(pools["ins"]),
            insurer_code_id=rng.choice(pools["ic"]), healthcare_network_id=rng.choice(pools["hn"]),
            domain_id=rng.choice(pools["dom"]), environment_id=rng.choice(pools["env"]),
        )
...
def parse_args(argv):
    ...
    p.add_argument("--duration",          type=int,   default=0,   help="seconds (0=infinite)")
    ...
    p.add_argument("--virtual-day-start", default="2026-03-24T06:30:00+01:00")
    p.add_argument("--virtual-speed",     type=float, default=120.0)
    p.add_argument("--anomaly-mode",      action="store_true",
                   help="inject ~25%% illogical/anomalous sessions for model validation")
...
def main(argv):
    ...
    try:
        nav_summary = rebuild_runtime_catalog(args.backend_apis, args.actions_order)
    except Exception as exc:
        print(f"Navigation config error: {exc}", file=sys.stderr)
        return 2
    ...
    if args.anomaly_mode:
        print("WARNING: anomaly-mode active; ~25% sessions will contain illogical sequences", file=sys.stderr)
    ...
    try:
        while True:
            ...
            # Refill queue if empty (new session)
            ...
            if args.dry_run:
                ...
                try:
                    ...
                    if args.async_send:
                        future.add_callback(lambda m: print(f"-> {m.topic}:{m.partition}@{m.offset}", flush=True))
                        future.add_errback(lambda e: print(f"send failed: {e}", file=sys.stderr, flush=True))
                    else:
                        m = future.get(timeout=max(1, args.request_timeout_ms // 1000))
                        print(f"-> {m.topic}:{m.partition}@{m.offset}  action={event['action']!r}  route={route}", flush=True)
                except Exception as exc:
