#!/usr/bin/env python3
"""
anomaly_kafka_producer_v2.py
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Updated Kafka producer aligned with simulator_yearly_dataset_v3.py.

Generates and publishes specific anomaly types to Kafka, optionally mixing
in normal sessions. Uses the same backend-apis-actions.json, actions_order-v2.json,
and anomaly generation logic as the training dataset.

Features:
  • Cycle through or pick specific anomaly types
  • Optional mix of normal sessions
  • Automatic feature engineering (sessionLength, timeDelta, risk scores, etc.)
  • Support for dry-run (stdout) or direct Kafka output
"""
...
def parse_args(argv):
    parser = argparse.ArgumentParser(
        description="Kafka anomaly producer aligned with training generators"
    )
    ...
    parser.add_argument(
        "--tag",
        choices=["all"] + training_sim.ANOMALY_TYPES,
        required=True,
        help="Anomaly type to emit. Use 'all' to cycle through all types.",
    )
    parser.add_argument(
        "--normal-ratio",
        type=float,
