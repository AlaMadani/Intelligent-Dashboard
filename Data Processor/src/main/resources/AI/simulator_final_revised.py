...
Improvements over the original:
  • Uses the same embedded action catalog as the dataset generator (no Java scan).
  • Each user follows a realistic route-based session: auth → [routes] → logout.
  • Events include 'route' and 'prevAction' fields (mirrors training data).
...
# ---------------------------------------------------------------------------
# Shared action catalog (identical to dataset generator)
# ---------------------------------------------------------------------------
...
ROUTE_WEIGHTS = [
    ("home",          0.30),("requests",  0.22),("refunds",     0.16),
    ("documents",     0.13),("beneficiaries",0.06),("personal_info",0.05),
    ("banking_info",  0.04),("preferences",0.03),("tp_card",     0.01),
]
...
def choose_status(route: str, rng: random.Random) -> str:
    failure_rates = {"auth":0.03,"logout":0.01,"banking_info":0.08,"documents":0.05,
                     "requests":0.05,"beneficiaries":0.04,"personal_info":0.03,
...
def choose_http_code(status, route, action_value, rng):
    ...
    failure_codes = {"auth":[401,403],"banking_info":[400,409,422],
                     "requests":[400,422],"documents":[400,422]}
...
def plan_new_session(user: UserProfile, rng: random.Random, anomaly_mode: bool) -> None:
    ...
    for v in login_values:
        a = ACTION_BY_VALUE[v]
        user.action_queue.append((a, "auth"))
...
def main(argv):
    ...
    if args.anomaly_mode:
        print("⚠  anomaly-mode active – ~25% sessions will contain illogical sequences", file=sys.stderr)
